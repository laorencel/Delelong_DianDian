package com.delelong.diandian.overlay;
		...
		try {
			...
			for (int i = 0; i < busSteps.size(); i++) {
				...
				if (i < busSteps.size() - 1) {
					...
					// 假如两个步行换乘中间没有公交，就把上一步行经纬度点和下一步行第一个经纬度点连接起来，避免断线问题
					// More
					// if (busStep.getWalk() != null && busStep.getBusLine() !=
					// null) {
					// LatLonPoint enterPoint = getFirstBuslinePoint(busStep);
					//
					// LatLng walkLatLng = AMapServicesUtil
					// .convertToLatLng(enterPoint);
					// LatLng busLatLng =
					// AMapServicesUtil.convertToLatLng(busStep
